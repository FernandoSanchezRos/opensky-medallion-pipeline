...
BRONZE_DEP_DEFAULT   = f"{LAKE_DEFAULT}/bronze/flights_departure"
SILVER_DEP_DEFAULT   = f"{LAKE_DEFAULT}/silver/flights_departure"
...
def maybe_register_table(spark, output_path: str, db: str = "silver", table: str = "flights_departure") -> None:
...
def main(
    ...
    spark = get_spark_session("Silver Flights Departure Transform")
    ...
    files = discover_new_files(spark, input_root=bronze_path, dataset="silver.flights_departure", pattern=pattern)
    if max_files:
        files = files[:max_files]
    if not files:
        print("[OK] No hay entradas nuevas para procesar en silver.flights_departure.")
        spark.stop()
        return
    ...
    # 5) Tiempos/calendarios y block time (dep = first, arr = last)
    df = (df
    ...
    # 7) Enriquecimiento (airlines y airports)
    ...
    df = (df.join(F.broadcast(ref_airlines), df.airline_prefix == ref_airlines.prefix, "left")
            .drop("prefix"))

    df = (df.join(F.broadcast(ref_airports).withColumnRenamed("icao","dep_icao_ref"),
                  df.dep_icao == F.col("dep_icao_ref"), "left")
            .withColumnRenamed("iata","dep_iata")
            .drop("dep_icao_ref"))

    df = (df.join(F.broadcast(ref_airports).withColumnRenamed("icao","arr_icao_ref"),
    ...
    # 9) Metadatos Silver
    df = add_silver_metadata(df, source_name="bronze.flights_departure", silver_version=silver_version)
    ...
    # 11) MERGE (partición por dep_date_utc)
    merge_upsert(spark, df, silver_path, on_expr="t.flight_id = s.flight_id", partition_cols=["dep_date_utc"])

    # 12) Métricas
    unknown_count = df.filter(F.col("unknown_dep") | F.col("unknown_arr")).count()
    m = capture_metrics(
        df, dataset="silver.flights_departure",
        source_key=f"batch://silver.flights_departure/{silver_run_id}",
    ...
    m = with_extras(m, {"departures_rows": after_dedupe})
    write_metrics(spark, m, rows_out=after_dedupe, status="ok", notes="silver.flights_departure upsert ok")

    # 13) Manifest
    update_manifest(spark, files=files, dataset="silver.flights_departure",
                    ingest_id=silver_run_id, status="done", silver_version=silver_version)

    # 14) Registrar tabla (opcional)
    if register_table:
        maybe_register_table(spark, silver_path, db="silver", table="flights_departure")

    print(f"[OK] silver.flights_departure: {after_dedupe} filas (dedupe de {before_dedupe}) → {silver_path}. run_id={silver_run_id}")
    spark.stop()


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Transformación SILVER de flights_departure (Bronze -> Silver)")
    parser.add_argument("--input", default=BRONZE_DEP_DEFAULT, help="Raíz BRONZE (p.ej. /opt/spark/lakehouse/bronze/flights_departure)")
    parser.add_argument("--output", default=SILVER_DEP_DEFAULT, help="Destino SILVER (p.ej. /opt/spark/lakehouse/silver/flights_departure)")
    ...
    parser.add_argument("--register-table", action="store_true", help="Registrar silver.flights_departure en el metastore si no existe")
