...
SCHEMA_FLIGHTS_ARRIVALS = T.StructType([
...
def maybe_register_table(spark, output_path: str, db: str = "bronze", table: str = "flights_arrival") -> None:
...
def main(input_root: str, output_path: str, max_files: int | None, register_table: bool) -> None:
    spark = get_spark_session("Bronze Flights Arrival Ingest")

    dataset = "flights_arrival"
    ...
    if not files:
        print("[OK] No hay ficheros nuevos que procesar (arrivals).")
        spark.stop()
        return

    print(f"[INFO] Arrivals a procesar ({len(files)}). Ejemplo: {files[:3]}")
    # 3) Leer NDJSON con esquema (modo PERMISSIVE para no reventar por sucios)
    df_raw = (
        spark.read
             .schema(SCHEMA_FLIGHTS_ARRIVALS)
             .option("mode", "PERMISSIVE")
             .option("columnNameOfCorruptRecord", "_corrupt_record")
             .json(files)
    )

    # 4) Metadatos comunes + extraer "tag" del path
    ...
    if register_table:
        maybe_register_table(spark, output_path, db="bronze", table="flights_arrival")

    # 7) Manifiesto -> done
    update_manifest(spark, files, dataset, ingest_id)

    print(f"[OK] flights_arrival: Procesados {len(files)} ficheros -> {output_path} (Delta) + manifiesto actualizado. ingest_id={ingest_id}")
    spark.stop()

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Ingesta BRONZE de flights arrival (RAW NDJSON -> Delta)")
    parser.add_argument("--input", required=True, help="Raíz RAW (p.ej. /opt/spark/raw-data/flights_arrival)")
    parser.add_argument("--output", required=True, help="Destino BRONZE (p.ej. /opt/spark/lakehouse/bronze/flights_arrival)")
    parser.add_argument("--max-files", type=int, default=None, help="(Opcional) límite de ficheros por ejecución")
    parser.add_argument("--register-table", action="store_true", help="Registrar bronze.flights_arrival en el metastore si no existe")
