# utils/init_silver_manifest.py
from pyspark.sql import functions as F, types as T
from delta.tables import DeltaTable
import glob, hashlib, os

# Tabla Delta del manifiesto de SILVER
META_PATH = "/opt/spark/lakehouse/meta/silver_runs"

# Esquema (similar a Bronze) + silver_version
MANIFEST_SCHEMA = T.StructType([
    T.StructField("dataset",        T.StringType(),   False),
    T.StructField("source_file",    T.StringType(),   False),
    T.StructField("ingest_id",      T.StringType(),   False),   # usa aquí tu run_id de Silver
    ...
    T.StructField("silver_version", T.StringType(),   True),  # versión del proceso Silver
])
...
def _paths_to_df(spark, files, dataset: str, ingest_id: str, status: str,
                 file_hash: str | None = None, silver_version: str | None = None):
    df = (spark.createDataFrame(files, T.StringType()).toDF("source_file")
          ...
          .withColumn("silver_version", F.lit(silver_version).cast(T.StringType())))
    return df.select("dataset","source_file","ingest_id","processed_ts","status","file_hash","silver_version")

# -------- API pública --------
def ensure_manifest(spark):
    try:
        spark.read.format("delta").load(META_PATH).limit(1).collect()
        print("[INFO] Silver manifest ya existe.")
    except Exception:
        print("[INFO] Creando Silver manifest vacío...")
        empty_df = spark.createDataFrame([], MANIFEST_SCHEMA)
        empty_df.write.format("delta").mode("overwrite").save(META_PATH)

def needs_processing(spark, dataset: str, source_file: str, silver_version: str) -> bool:
    """
    True si NO hay registro 'done' para (dataset, source_file) con esa misma silver_version.
    Es decir, re-procesa si es nuevo o la versión no coincide.
    """
    try:
        df = (spark.read.format("delta").load(META_PATH)
              .filter( (F.col("dataset")==dataset) &
                       ...
                       (F.col("status")=="done") &
                       (F.col("silver_version")==silver_version) ))
...
def discover_new_files(spark, input_root: str, dataset: str, pattern: str = "date=*/*.parquet"):
    """
    Igual que en Bronze: devuelve paths de input_root que NO están 'done' (cualquier versión).
    Útil si tu input de Silver es un conjunto de ficheros/particiones.
    """
...
def update_manifest(spark, files, dataset: str, ingest_id: str,
    ...
    Si re-procesas con nueva silver_version, la fila se actualiza con la versión nueva.
    """
    if not files:
        return
    delta = _as_delta(spark)
    df_new = _paths_to_df(spark, files, dataset, ingest_id, status, file_hash, silver_version)

    delta.alias("t").merge(
        df_new.alias("s"),
        "t.dataset = s.dataset AND t.source_file = s.source_file"
    ).whenMatchedUpdate(
        condition="t.status <> s.status OR t.file_hash <> s.file_hash OR t.ingest_id <> s.ingest_id OR t.silver_version <> s.silver_version",
        set={
            "ingest_id":      "s.ingest_id",
            "processed_ts":   "s.processed_ts",
            "status":         "s.status",
            "file_hash":      "s.file_hash",
            "silver_version": "s.silver_version",
        }
...
def snapshot_needs_processing(spark, dataset: str, file_path: str, silver_version: str) -> bool:
    """
    True si (a) el MD5 cambió o (b) no existe 'done' con esta silver_version.
    """
    ...
    try:
        ...
        same_ver  = ( (last[0]["silver_version"] or "") == silver_version )
